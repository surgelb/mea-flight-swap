...
export async function POST(req: Request) {
  try {
    ...
    result = parseRosterTextProgrammatic(rawText);

    if (result) {
      console.log('[Register Roster API] Programmatic PDF parsing succeeded!');
    } else {
      console.log('[Register Roster API] Programmatic parser returned null, falling back to OpenRouter LLM...');
    ...
    if (!result || !result.isValidRoster) {
      return NextResponse.json({ 
